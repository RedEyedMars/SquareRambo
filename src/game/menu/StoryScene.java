...
import game.Square;
import game.WinnableEndlessMode;
import gui.Gui;
...
public class StoryScene extends GraphicView{
	...
	public static void setupScenes(){
		...
		Hub.scenes.add(new Action(){public void act(Object o){Gui.setView(new WinnableEndlessMode(300){
			...
			{				
				for(float y = 0f;y<=1.0375f;y+=0.0375f){
					Square left = new Square("black",1f);
					left.setSide("red");
					addSquare(left);
					borders.add(left);
					Square right = new Square("black",1f);
					right.setSide("red");
					addSquare(right);
					borders.add(right);
				...
				Square regenerator = new Square("black",1.0f){
					private int index=0;
					@Override
			...
			private void generateSideRail(float y, Square left, Square right){
				if(left!=null){
					left.adjust(borderX-0.2f,0.0376f);
					left.setY(y);
					Square botLeft = new Square("black",0f);
				...
				if(right!=null){
					...
					Square botRight = new Square("black",0f);
		...
		//7
		Hub.scenes.add(new Action(){public void act(Object o){Gui.setView(new BattleMode(){
			private Square leftTop = new Square("black",0.05f);
			private List<Square> walls = new ArrayList<Square>();
			...
			private void makeWall(int i){
				final Square destination = new Square("black",0.05f);
				destination.setX(i*0.05f);
				Square wall = new Square("red",0.05f){
					@Override
					public Action<Square> getOnHitAction(Square hitter){
